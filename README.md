...
|`ACTIVE_HEALTHCHECKS=0/1`|The container uses a very basic Docker [healthcheck](https://docs.docker.com/reference/dockerfile/#healthcheck) [script](healthcheck.sh) that can be used to ensure the VPN is up before starting other services. By default only passive checks that don't generate any traffic are run. Set this to 1 to also allow checks that generate traffic, such as `ping`, in order to detect if the remote endpoint is responding. Note this setting is not relevant if custom healthchecks are defined (eg. using [Docker Compose](https://docs.docker.com/reference/compose-file/services/#healthcheck)).
|`HEALTHCHECK_PING_TARGET`|When active healthchecks are enabled or reconnect logic is used, this can be used to override the target/s that gets pinged when testing that the endpoint is still responding. Can be set to space or comma separated list of multiple targets, in which case all need to fail for the endpoint to be considered unresponsive. Defaults to `www.privateinternetaccess.com 1.1.1.1`.
...
`ALLOW_MISSING_IPTABLES_RULES`|Some hosts may be missing `iptables` related modules needed by `wg-quick`, causing it to fail when bringing up the WireGuard interface. Some users have found the container to still function regardless, however some `iptables` rules set by `wg-quick` will be missing. Setting this var to `1` will treat any missing modules as a non-fatal error on startup. This var is only added as a way to revert to previous (unintended) behavior if desired, and isn't ideal. Enabling any missing modules on the host itself is probably the better solution. See [issue #179](https://github.com/thrnz/docker-wireguard-pia/issues/179#issuecomment-4755980540).
...
WireGuard keys may also expire if a second set of keys is registered to the same endpoint address as a previous set using the same auth token. In practice this is probably only an issue if multiple containers share the same auth token, and if they also happen to pick the same vpn endpoint.

The container has optional recovery logic if the remote endpoint permanently stops responding. In order to register new WireGuard keys, temporary routes and firewall rules are added to allow some requests to bypass the tunnel. To prevent potential leaks while this happens, these requests are limited to a certain interface, ports, and root UID. It's probably best to ensure processes in other containers are run as non-root UIDs. If successful, new WireGuard settings are then applied without needing to bring the existing interface down. Apart from a change in address (and possibly forwarded port number if used) the process should be mostly transparent to other containers sharing the connection. As on startup, only a single attempt is made to register new WireGuard keys, and it relies on `curl` retrying transient connectivity issues. The recovery logic is disabled by default, and is enabled by setting the `RECONNECT=1` env var.
