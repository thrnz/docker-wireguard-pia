...
WireGuard keys may also expire if a second set of keys is registered to the same endpoint address as a previous set (presumably by the same account). Generally each location has a pool of ip addresses available and a random one is chosen, however if two containers happen to be created that end up sharing the same endpoint address, then the endpoint in the older container may stop responding.
