...
The container has optional recovery logic if the remote endpoint permanently stops responding. In order to register new WireGuard keys, temporary routes and firewall rules are added to allow some requests to bypass the tunnel. To prevent potential leaks while this happens, these requests are limited to a certain interface, ports, and root UID. It's probably best to ensure processes in other containers aren't run as root. If successful, new WireGuard settings are then applied without needing to bring the existing interface down. Apart from a change in address (and possibly forwarded port number if used) the process should be mostly transparent to other containers sharing the connection. As on startup, only a single attempt is made to register new WireGuard keys, and it relies on `curl` retrying transient connectivity issues. The recovery logic is disabled by default, and is enabled by setting the `RECONNECT=1` env var.
