...
`LOC_FILE=/path/to/loc-file`|`LOC` values can be read in via a file if needed. This allows for changing the value once the container is already running when reconnecting. The file format should match that of `LOC` - a single line of comma or space separated values.
